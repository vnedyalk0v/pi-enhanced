import type { WorkflowDef } from "./domain.ts";

/**
 * Default four-phase repository task:
 * reconnaissance → implementation → review → synthesis
 *
 * Parallel slots stay within the subagent concurrency cap (4).
 */
export function defaultRepoTaskTemplate(): WorkflowDef {
  return {
    name: "repo-task",
    phases: [
      {
