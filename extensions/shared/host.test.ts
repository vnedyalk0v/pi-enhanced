...
function makeCtx(widgets: Map<string, unknown>) {
  return {
    hasUI: true,
    ui: {
...
function makeHost(overrides?: { getRunning?: () => number | undefined; dispose?: () => Promise<void> }) {
  ...
  const host = createManagerHost<{ id: string; label: string }>(pi, {
    widgetId: "test-widget",
    customType: "test-result",
    runningLabel: (n) => `${n} running`,
    completion: (snap) => ({ content: `done: ${snap.label}`, details: { id: snap.id } }),
